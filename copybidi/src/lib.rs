...
// Ref: https://github.com/tokio-rs/tokio/blob/master/tokio/src/io/util/copy_bidirectional.rs

pub use crate::errors::CopyBidiError;
use crate::{copy::CopyBuffer, errors::CopyError};
    ...
    loop {
        match state {
            ...
            TransferState::ShuttingDown(count) => {
                ready!(w.as_mut().poll_shutdown(cx).map_err(CopyError::write))?;
...
pub async fn copy_bidirectional<A, B>(a: &mut A, b: &mut B) -> Result<(u64, u64), CopyBidiError>
where
    A: AsyncRead + AsyncWrite + Unpin + ?Sized,
    B: AsyncRead + AsyncWrite + Unpin + ?Sized,
{
    copy_bidirectional_impl(
        a,
        b,
        CopyBuffer::new(crate::DEFAULT_BUF_SIZE),
        CopyBuffer::new(crate::DEFAULT_BUF_SIZE),
    )
    .await
}
    ...
    B: AsyncRead + AsyncWrite + Unpin + ?Sized,
{
    ...
    poll_fn(|cx| {
        let a_to_b =
            transfer_one_direction(cx, &mut a_to_b, a, b).map_err(CopyBidiError::a_to_b)?;
        let b_to_a =
            transfer_one_direction(cx, &mut b_to_a, b, a).map_err(CopyBidiError::b_to_a)?;
